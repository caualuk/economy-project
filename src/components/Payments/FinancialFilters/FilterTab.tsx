...
export default function FilterTab(): JSX.Element {
  const tabs: Tab[] = [
    { key: "todos", label: "Todos" },
    { key: "pagos", label: "Pagos", count: 3 },
  ...
  return (
    <div
      ...
      role="tablist"
      aria-label="Filtros"
      className="relative flex gap-2 p-2 m-1 rounded-xl bg-[#e7e5e5]"
    >
      {/* Indicador */}
      <div
        aria-hidden
        className="absolute top-1 bottom-1 rounded-lg bg-white shadow transition-all duration-300"
      ...
      {tabs.map((t, i) => (
        <button
          ...
          className={`relative z-10 flex h-6 items-center gap-2 rounded-lg px-4 text-sm
            border-none
            outline-none
            focus:border-none
            focus:outline-none
            ...
            active:outline-none
            cursor-pointer
            ${active === i ? "text-black" : "text-gray-600"}`}
